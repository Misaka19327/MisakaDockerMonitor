...
export class ClickHouseStorage implements StorageAdapter {
  ...
  async getInstances(containerId: string): Promise<ContainerInstance[]> {
    const result = await this.client.query({
      query: `SELECT * FROM container_instances WHERE container_id = '${escapeClickHouseString(containerId)}' ORDER BY started_at DESC`,
      format: 'JSONEachRow',
    })
    const rows = await result.json()
  ...
  async getActiveInstance(containerId: string): Promise<ContainerInstance | null> {
    const result = await this.client.query({
      query: `SELECT * FROM container_instances WHERE container_id = '${escapeClickHouseString(containerId)}' AND status = 'running' ORDER BY started_at DESC LIMIT 1`,
      format: 'JSONEachRow',
    })
    const rows = await result.json()
  ...
  async getDistinctLevels(containerId: string): Promise<string[]> {
    const result = await this.client.query({
      query: `SELECT DISTINCT level FROM log_entries WHERE container_id = '${escapeClickHouseString(containerId)}' AND level IS NOT NULL ORDER BY level`,
      format: 'JSONEachRow',
    })
    const rows = await result.json()
  ...
  async getDistinctFieldValues(containerId: string, field: string): Promise<string[]> {
    const safeField = assertSafeFieldName(field)
    const result = await this.client.query({
      query: `SELECT DISTINCT JSONExtractString(parsed_json, '${safeField}') as val FROM log_entries WHERE container_id = '${escapeClickHouseString(containerId)}' AND is_json = 1 ORDER BY val LIMIT 100`,
      format: 'JSONEachRow',
    })
    const rows = await result.json()
