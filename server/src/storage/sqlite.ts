...
export class SqliteStorage implements StorageAdapter {
    ...
    async insertLog(entry: LogEntry): Promise<void> {
        if (!this.insertStmt) {
            this.insertStmt = this.db.prepare(
                `INSERT INTO log_entries (container_id, container_name, instance_id, timestamp, line_number,
    ...
    `)
        try {
            this.db.run('ALTER TABLE container_instances ADD COLUMN watched INTEGER NOT NULL DEFAULT 1')
        } catch {
        }
